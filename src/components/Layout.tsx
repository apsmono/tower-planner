...
interface NavItem {
  ...
  name: string;
  icon?: any;
  currency?: 'coins' | 'cells' | 'gems' | 'stones' | 'shards' | 'cash';
  color: string;
}
...
export function Layout({ activeTab, setActiveTab, children }: LayoutProps) {
  ...
  // Left sidebar collapse state with localStorage persistence
  const [isLeftCollapsed, setIsLeftCollapsed] = useState<boolean>(() => {
    try {
      const saved = localStorage.getItem('tower_left_sidebar_collapsed');
  ...
  // Right sidebar collapse state with localStorage persistence
  const [isRightCollapsed, setIsRightCollapsed] = useState<boolean>(() => {
    try {
      const saved = localStorage.getItem('tower_right_sidebar_collapsed');
      return saved ? JSON.parse(saved) : false;
    } catch {
      return false;
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('tower_left_sidebar_collapsed', JSON.stringify(isLeftCollapsed));
    } catch {
      // ignore
    }
  }, [isLeftCollapsed]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('tower_right_sidebar_collapsed', JSON.stringify(isRightCollapsed));
    } catch {
      // ignore
    }
  }, [isRightCollapsed]);
  ...
  const navItems: NavItem[] = [
    { id: 'runs', name: 'Import & Runs', icon: History, color: 'text-blue-400 hover:text-blue-300' },
    { id: 'tier-lab', name: 'Tier Lab', icon: TrendingUp, color: 'text-indigo-400 hover:text-indigo-300' },
    { id: 'research-queue', name: 'Research Queue', icon: FlaskConical, color: 'text-emerald-400 hover:text-emerald-300' },
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-950 text-zinc-100 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {user?.isLoggedIn ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-xs font-semibold text-white shadow-xs transition-all cursor-pointer"
            >
      ...
      {/* ── Persistent Cloud Sync / Adoption Banner ──────────────── */}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {isLeftCollapsed ? (
          /* Collapsed Left Rail (52px) */
          <aside className="w-13 shrink-0 bg-zinc-900/40 border-r border-zinc-800/80 p-2 flex flex-col justify-between items-center glass-panel transition-all duration-300 z-10 select-none">
            <div className="flex flex-col items-center space-y-3 w-full">
              {/* Expand button replacing brand logo in rail */}
              <button
              ...
              <nav className="flex flex-col space-y-2 w-full items-center">
                {navItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {item.currency ? (
                        <span className={`w-4 h-4 flex items-center justify-center ${isActive ? 'scale-110' : 'group-hover:scale-110'} transition-transform`}>
                          <CurrencyIcon currency={item.currency} size="xs" />
                        </span>
                      ) : (
                        Icon && <Icon className={`w-4 h-4 ${item.color} ${isActive ? 'scale-110' : 'group-hover:scale-110'} transition-transform`} />
                      )}
                    </button>
                  );
                })}
                ...
                <div className="flex items-center space-x-2.5">
                  <div className="w-7 h-7 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="28" height="28">
                      ...
                      <polygon points="50,6 88,28 88,72 50,94 12,72 12,28" fill="none" stroke="#ff3300" strokeWidth="2.8" opacity="0.95"/>
              ...
              <nav className="space-y-1">
                {navItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {item.currency ? (
                        <span className={`w-4 h-4 flex items-center justify-center shrink-0 ${isActive ? 'scale-110' : ''}`}>
                          <CurrencyIcon currency={item.currency} size="xs" />
                        </span>
                      ) : (
                        Icon && <Icon className={`w-4 h-4 ${item.color} ${isActive ? 'scale-110' : ''}`} />
                      )}
                      <span>{item.name}</span>
                    </button>
                  );
